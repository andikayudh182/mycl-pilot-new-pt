...
<section class="m-5">
    ...
    <h5> Total Mylea Belum Kerik :{{ $Total }}</h5>
    <h5> Total Mylea Sudah Kerik :{{ $TotalSudahKerik }}</h5>
    <h5> Total WIP Post Treatment :{{ $TotalPTProses  }}</h5>
    <h5> Total Reject Panen :{{ $TotalRejectPanen  }}</h5>
    <h5> Total Reject Kerik :{{ $TotalRejectKerik  }}</h5>
    <h5> Total Sisa Belum Terpakai :{{ $TotalSisaBelumTerpakai  }}</h5>
    
       
    <form action="{{url('/operator/post-treatment/data-panen')}}" method="GET">
        <p>
    ...
    <table class="table">
        <tr class="sticky-header">
            ...
            <th>Hasil Kerik</th>
            <th>Reject Kerik</th>
            <th>Stok </br>(Belum Dikerik)   </th>
            <th>Batch Post </br> Treatment</th>
            <th>Sisa Belum Terpakai </br> (Hasil Kerik - Total qty Pemakaian PT)</th>
            <th> Aksi </th>
        </tr>
        ...
        @foreach($Trial as $data)
            <tr id = {{"Panen".$data['id']}}>
                ...
                @endif
                <td>{{$data['kerik']->sum('Jumlah')-$data['PostTreatment']->sum('Jumlah')}}</td>
                <td>
